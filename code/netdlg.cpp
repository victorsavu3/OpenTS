...
void Destroy_Connection(int id, int error)
{
	...
	txt[0] = '\0';
	if (error==1) {
		snprintf(txt, sizeof(txt), Fetch_String(TXT_CONNECTION_LOST), Session.Shown_Name(housep).c_str());
	} else if (error==0) {
		snprintf(txt, sizeof(txt), Fetch_String(TXT_LEFT_GAME), Session.Shown_Name(housep).c_str());
	}
