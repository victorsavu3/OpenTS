...
- **Dialogs and window messages.** The interface screens are RmlUi documents,
  which answer with `UIResult` from `code/ui/uiscreen.h`. The network lobby
  uses message names of its own (`code/lobbymsg.h`), which keep the Win32
  numbers, and reads the Westwood serial through `platform/registry.h`.
